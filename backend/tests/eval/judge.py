...
9 维度评分系统（6 基础 + 3 多轮专用），每维度 1-5 分。

核心组件:
- JudgeDimension: 9 个评分维度枚举
- JudgeScore: 单维度评分结果
- JudgeResult: 完整评分结果（9 维度 + 总分 + 加权平均）
...
DEFAULT_DIMENSION_WEIGHTS: Dict[str, float] = {
    # 基础维度权重
    "tool_selection": 0.15,
    "param_extraction": 0.12,
    "response_quality": 0.12,
    "error_handling": 0.10,
    "efficiency": 0.08,
    "user_experience": 0.10,
    # 多轮专用维度权重
    "context_retention": 0.12,
    "follow_up_quality": 0.10,
    "conversation_coherence": 0.11,
}
...
class JudgeResult:
    ...
    def max_possible_score(self) -> int:
        """最大可能分数"""
        return len(self.scores) * 5 if self.scores else 45
...
class LLMJudge:
    ...
    async def evaluate(
        ...
        if self.inject_llm_response is not None:
            ...
            # 无注入时，返回默认中间分（框架模式，不调用真实 LLM）
            raw_output = self._generate_default_response()

        # 解析输出
        parsed = self._parse_llm_output(raw_output)
        return self._build_result_from_parsed(parsed, test_id, raw_output)
