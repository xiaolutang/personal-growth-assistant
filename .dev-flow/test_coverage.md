...
| 在线状态检测+离线启动恢复 | S04 | unit | useOnlineStatus 返回正确状态/offline 事件触发/online 事件触发/OfflineIndicator 离线显示/上线 3 秒消失/网络失败不 logout 保留 token/401 无效 token 仍 logout/ProtectedRoute token 存在即放行/无 token 跳转登录 | pending | risk: startup |
