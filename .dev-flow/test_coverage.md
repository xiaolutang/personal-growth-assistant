...
| Module | Task IDs | Test Type | Required Scenarios | Status | Gaps |
|--------|----------|-----------|--------------------|--------|------|
| AI 深度洞察 API | S15 | unit | 有数据返回结构化洞察/空数据降级/LLM 不可用走规则/用户隔离 | completed | risk: network, 9 tests in test_review_api.py |
| AI 深度洞察卡片 | F108 | unit+manual | 3 维度渲染/skeleton/降级标签/日报隐藏/API 失败错误态 | completed | F2 验证, 347 frontend tests + build pass |
